...
export default function Sidebar(){
    return (
        <aside className="w-3/12 h-full bg-slate-100 fixed p-7">
            {/* navbar header */}
            <div className="">
                <img src={meow} className='rounded-full w-20 h-20' />
                <span className="block mt-5 text-slate-700 font-medium text-lg">Selamat malam meooww</span>
            </div>
            ...
            <ul className='grid gap-3'>
                ...
                <li>
                    <NavLink to="/add" className={({ isActive }) => (isActive ? 'nav-item active-nav' : 'nav-item inactive-nav')}>Catatan Baru</NavLink>
                </li>
                <li>
